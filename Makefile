...
help:
	@echo "EstateIQ â€” common targets"
	@echo ""
...
tools-deploy:  ## Deploy every OSS tool registered for EstateIQ via ArgoCD
	kubectl apply -f gitops/argocd/applicationsets/all-tools-applicationset.yaml
	argocd appset get estateiq-tools --grpc-web

tools-status:  ## Show sync status of every registered tool
	argocd app list -l "project=estateiq-tools" --grpc-web
