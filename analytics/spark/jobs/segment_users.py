...
spark = SparkSession.builder.appName("estateiq-segment").getOrCreate()
df = spark.read.format("iceberg").load("estateiq_mart.user_features")
...
preds.select("user_id","prediction").writeTo("estateiq_mart.user_segments").using("iceberg").createOrReplace()
